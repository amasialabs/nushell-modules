...
```
When touching persistence, clear state with `rm ($nu.data-dir | path join "amasia-data" "snip" "snip.json")` before retesting.
...
Runtime state persists in `($nu.data-dir | path join "amasia-data" "snip" "snip.json")`; never commit that file. Snippet sources may contain secrets, so redact paths in examples. Clipboard helpers shell out to system binaries; feature-detect new integrations instead of assuming availability, and guard platform checks with `which`.
